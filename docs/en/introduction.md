...
[中文版](https://czyt1988.github.io/SARibbon/zh/)
...
- Use `doxygen` to generate HTML or `.qch` documentation:
  - `docs/doxygen-doc-file/Doxyfile-wiki-en` → Generate web documentation *(Note: the `-en` variant does not yet exist — use `Doxyfile-wiki-cn` as a reference, or create the `-en` version from it.)*
  - `docs/doxygen-doc-file/Doxyfile-qch-en` → Generate Qt Creator help documentation *(Note: the `-en` variant does not yet exist — use `Doxyfile-qch-cn` as a reference, or create the `-en` version from it.)*
