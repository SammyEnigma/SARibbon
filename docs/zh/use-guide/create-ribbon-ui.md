...
SARibbonPanel支持三种布局方案，分别是3行模式、2行模式和单行模式（v2.8.0 新增），通过`SARibbonBar::setRibbonStyle`函数你可以设置不同的布局风格
...
对于单行模式（v2.8.0 新增），所有按钮排列在同一行内，图标在左侧、文字在右侧横向排列，Large、Medium、Small 占位都等效为 Small。建议搭配 `SARibbonBar::setEnableIconRightText(true)` 使用。
...
为了区分上下文标签页和普通的标签页，上下文标签页会有特殊的颜色进行标识，如上图所示。这个标识的颜色可以通过`SARibbonBar::setContextCategoryColorList`设置
