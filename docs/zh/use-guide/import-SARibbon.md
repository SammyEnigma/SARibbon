...
## 直接引入方式的完整CMakeLists.txt示例

以下是一个直接将 `SARibbon.h` 和 `SARibbon.cpp` 嵌入到工程中的完整 CMakeLists.txt 配置（参考自 `example/StaticExample`）：
...
!!! tip "提示"
    直接嵌入方式最为简单，适合快速上手。如果项目规模较大或需要多个项目共享 SARibbon，建议采用编译为动态库后通过 `find_package` 引入的方式。
